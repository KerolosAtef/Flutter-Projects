...
class Room3 extends StatefulWidget{
  ...
  State<StatefulWidget> createState() {
    ...
    return new Room3State();
  }

}
class Room3State extends State<Room3> {
    ...
    if (swNum==9){
      ...
      setState(() {
        if (value) {
          ...
          sendSignal("3", swNum.toString(), "on");
          if (++numOn==8){
            switchNum[8]=true;
          }
          ...
          sendSignal("3", swNum.toString(), "off");
          if (--numOn<8){
            switchNum[8]=false;
          }
...
//      _onChildRemoved=roomSwitch.onChildRemoved.listen(_onChildRemovedFunction);
    ...
    setInitialDataFromPreferences("room3Sw");
    setInitialDataFromPreferences("room3Icons");
...
//        print(e.snapshot.value);
      ...
      setState(() {
        ...
        roomSwitch=FirebaseDatabase.instance.reference().child("Licenses")
            .child(myLicence).child("Rooms").child("Room3");
  ...
  editAll(){
    if (!canEdit){
      ...
      setState(() {
        ...
        saveData("room3Sw",switchesTexts);
        saveData("room3Icons",lampStatus);
      });
    }
  }
          ...
          child:  new Column(
            children: <Widget>[
              new ClipRRect(
                ...
                child: Image.asset(
                  "assets/images/bedroom2.png",fit: BoxFit.cover,scale: 1,
                ),
              ),
              new Flexible(child:
