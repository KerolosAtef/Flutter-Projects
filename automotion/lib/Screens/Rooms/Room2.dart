    ...
    if (swNum==9){
      ...
      setState(() {
        if (value) {
          ...
          sendSignal("2", swNum.toString(), "on");
          if (++numOn==8){
            switchNum[8]=true;
          }
        }
          ...
          sendSignal("2", swNum.toString(), "off");
          if (--numOn<8){
            switchNum[8]=false;
          }
...
//      _onChildRemoved=roomSwitch.onChildRemoved.listen(_onChildRemovedFunction);
    ...
    setInitialDataFromPreferences("room2Sw");
    setInitialDataFromPreferences("room2Icons");
    ...
    if (e.snapshot.key=="ip") {
      ...
      setState(() {
        ...
        roomSwitch=FirebaseDatabase.instance.reference().child("Licenses")
            .child(myLicence).child("Rooms").child("Room2");
  ...
  editAll(){
    if (!canEdit){
      ...
      setState(() {
        ...
        saveData("room2Sw",switchesTexts);
        saveData("room2Icons",lampStatus);
      });
    }
  }
          ...
          child:  new Column(
            children: <Widget>[
              new ClipRRect(
                ...
                child: Image.asset(
                  "assets/images/bedroom1.png",fit: BoxFit.cover,scale: 1,
                ),
              ),
              new Flexible(child:
